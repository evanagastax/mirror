  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.textPrimary }]}>Impact</Text>
        <Pressable
      ...
      <View style={[styles.statCard, { backgroundColor: colors.bgCard, borderColor: colors.border }]}>
        <View style={styles.statTop}>
          <View>
            <Text style={[styles.statLabel, { color: colors.textMuted }]}>IMPACT SCORE</Text>
            <Text style={[styles.statValue, { color: colors.textPrimary }]}>{pillars?.impact ?? "—"}</Text>
          </View>
          <View style={styles.levelBadge}>
            <Text style={[styles.levelLabel, { color: colors.textMuted }]}>LVL</Text>
        ...
        <Text style={[styles.xpText, { color: colors.textMuted }]}>{xp} / {xpMax} XP to next level</Text>
      </View>
      ...
      <View style={styles.actionRow}>
        ...
        >
          <Text style={[styles.actionIcon, { color: COLOR }]}>◈</Text>
          <Text style={[styles.actionLabel, { color: colors.textSecondary }]}>Roadmap</Text>
        ...
        >
          <Text style={[styles.actionIcon, { color: COLOR }]}>⟳</Text>
          <Text style={[styles.actionLabel, { color: colors.textSecondary }]}>Sync GitHub</Text>
          <Text style={[styles.actionArrow, { color: colors.textMuted }]}>›</Text>
        </Pressable>
      </View>
      ...
      {isLoading ? (
        ...
        <View style={styles.center}>
          <Text style={[styles.errorText, { color: colors.textSecondary }]}>Couldn't load logs.</Text>
        ...
        <View style={styles.emptyWrap}>
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>No impact logs yet.</Text>
          <Text style={[styles.emptySub, { color: colors.textMuted }]}>Log a PR, shipped feature, or completed task.</Text>
        </View>
      ) : (
        <View style={styles.logList}>
          {impactLogs.slice(0, 20).map((log) => {
            ...
            return (
              <View key={log.id} style={[styles.logRow, { backgroundColor: colors.bgCard, borderColor: colors.border }]}>
                <View style={[styles.logDot, { backgroundColor: BG }]}>
                  <Text style={{ fontSize: 14, color: COLOR }}>◈</Text>
                </View>
                <View style={styles.logInfo}>
                  <Text style={[styles.logTitle, { color: colors.textPrimary }]} numberOfLines={1}>{title}</Text>
                  <Text style={[styles.logSub, { color: colors.textMuted }]}>{formatDate(log.created_at)}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  retryBtn: { paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderRadius: 8 },
  retryText: { fontSize: 13 },
  header: { flexDirection: "row", alignItems: "center", gap: 12, marginBottom: 20 },
  back: { fontSize: 15 },
  title: { flex: 1, fontSize: 20, fontWeight: "700" },
  logBtn: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 14, paddingVertical: 7 },
  logBtnText: { fontSize: 13, fontWeight: "500" },
  statCard: { borderWidth: 1, borderRadius: 16, padding: 18, marginBottom: 16, gap: 10 },
  ...
  statValue: { fontSize: 44, fontWeight: "800", letterSpacing: -1, marginTop: 2 },
  levelBadge: { alignItems: "center" },
  levelLabel: { fontSize: 9, fontWeight: "700", letterSpacing: 1 },
  levelNum: { fontSize: 28, fontWeight: "800" },
  xpTrack: { height: 5, borderRadius: 99, overflow: "hidden" },
  xpFill: { height: 5, borderRadius: 99 },
  xpText: { fontSize: 11 },
  actionRow: { gap: 8, marginBottom: 24 },
  actionBtn: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderRadius: 12, padding: 14 },
  ...
  logList: { gap: 8 },
  logRow: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderRadius: 12, padding: 12 },
  logDot: { width: 36, height: 36, borderRadius: 18, alignItems: "center", justifyContent: "center" },
  logInfo: { flex: 1 },
  logTitle: { fontSize: 14, fontWeight: "500" },
